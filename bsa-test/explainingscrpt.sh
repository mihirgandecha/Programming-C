...
# Try and compile the code (sanitized version)
make driverbsa_s
if [[ -f driverbsa_s ]]
then
...
# timeout of 60 seconds in case of infinite loop - you can change to slightly longer
# output saved in out.txt
timeout 60 ./driverbsa_s >> out.txt
r=$?
case "$r" in
#checking what is being returned from compiling:
   0)
...
#Checking if out.txt is empty. Because output from sanitize should be empty:
#meaning REMOVE PRINT STATEMENTS FROM DRIVER.C!!! can you run in .c? 
if [[ ! -s out.txt ]]
then
...
make fibmemo
if [[ -f fibmemo ]]
    #no cases! you might want to change according to your cases
then
...
#Looking for "1 => 1@2 => 1@3 => 2@" output sequence <- if yes PASS
n=`cat out.txt| tr '\n' '@'|egrep "1 => 1@2 => 1@3 => 2@"`
if [[ ! ${#n} -eq 0 ]]
then 
...
#extension: is factorial....
#main thing is checking the .zip file contents  
#what you are aiming to do - why?